			...
			<nav role="navigation">
				<ul id="browse-classes">
					...
					<#list f.values?values as value>
						...
						<li id="${value.id?html}"><a href="${urls.currentPage}?filters_main=${searchFilter?url + ":"?url + value.id?url}" data-uri="${value.id?html}" <#if value.selected> class="selected" </#if> >${valueLabel?html}<span class="count-classes">(${value.count})</span></a></li>
					</#list>
				</ul>
				<#if filters[indexFilterName]??>
					...
					<nav id="alpha-browse-container" role="navigation">
					    ...
					    <#assign indexString = i18n().alphabetical_index >
					    <#list 0..<indexString?length as i>
						    <#assign c = indexString[i]>
						    <li><a 
							     <#if indexFilter.inputText?contains(c)>
							     	class="selected" 
							     </#if>
						    href="${urls.currentPage}?filters_main=${searchFilter?url + ":"?url + selectedValue?url}&filter_input_${indexFilterName}=${"%5B" + c?lower_case?url + c?upper_case?url + "%5D.*"}" title="${i18n().browse_all_starts_with(c)}">${c}</a></li>
						</#list>
					    </ul>
					</nav>
				</#if>
