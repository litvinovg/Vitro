    ...
    <#if filters[indexFilterName]??>
        ...
        <nav id="alpha-browse-container" role="navigation">
            ...
            <#list i18n().browse_results_alphabetical_index?split(",") as c>
                <#assign regexValue = "%5B" + c?lower_case?url + c?upper_case?url + "%5D.*">
                <li><a 
                 <#if indexFilter.inputText?contains(c)>
                     class="selected" 
                 </#if>
                     href="${urls.currentPage}?filters_main=${searchFilter?url + ":"?url + selectedValue?url}&filter_input_${indexFilterName}=${regexValue}" title="${i18n().browse_all_starts_with(c)}">${c}</a>
                </li>
            </#list>
            </ul>
        </nav>
